...
export async function GET(request) {
  try {
    ...
    if (type === "general") {
      ...
      filter.inquirerUserId = null;
    } else if (type === "listing") {
      // For listing-specific inquiries
      filter.inquirerUserId = authResult.user._id;
      filter.listingId = { $ne: null }; // Ensure listingId exists
    } else {
      // Default: get all inquiries for the authenticated user
      filter.$or = [
        { inquirerUserId: authResult.user._id }, // Inquiries for user's listings
        { inquirerUserId: null }, // General inquiries (you might want to restrict this based on user role)
      ];
    }
